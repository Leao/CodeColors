...
class ColorHandler implements CcColorStateList.ColorGetter<ColorHandler>, CcColorStateList.ColorSetter, Parcelable {
    ...
    private ColorHandler(int stateCapacity) {
    ...
    public ColorHandler(ColorHandler orig) {
        this(orig.mStateIndexValueList.size());
        setTo(orig);
    }

    public void setTo(ColorHandler orig) {
        mColor = orig.mColor;
    ...
    public ColorHandler(Parcel source) {
        setColor((ColorStateList) source.readParcelable(ColorHandler.class.getClassLoader()));
        int N = source.readInt();
        for (int i = 0; i < N; i++) {
            if (source.readByte() == 1) {
                setState(source.createIntArray(), (Integer) source.readValue(ColorHandler.class.getClassLoader()));
            }
        }
    }
    ...
    @Override
    public ColorHandler withAlpha(int alpha) {
    ...
    public static final Parcelable.Creator<ColorHandler> CREATOR = new Parcelable.Creator<ColorHandler>() {
        @Override
        public ColorHandler[] newArray(int size) {
            return new ColorHandler[size];
        }

        @Override
        public ColorHandler createFromParcel(Parcel source) {
            return new ColorHandler(source);
        }
    };
}
